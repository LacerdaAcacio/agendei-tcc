...
describe('useWishlist', () => {
  ...
  it('should fetch favorited services on mount', async () => {
    ...
    expect(result.current.favoritedServices).toHaveLength(2)
  })
  ...
  it('should handle add to wishlist', async () => {
    ...
    await result.current.toggleFavorite('service-123')
  ...
  it('should handle remove from wishlist', async () => {
    ...
    await waitFor(() => {
      expect(result.current.favoritedServices).toHaveLength(1)
    })
  ...
  it('should handle API error gracefully', async () => {
    ...
    await waitFor(() => {
      expect(result.current.isLoading).toBe(false)
      expect(result.current.favoritedServices).toEqual([])
    })
  })
})
